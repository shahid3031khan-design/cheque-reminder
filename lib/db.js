...
function callsCol() { return db.collection("calls"); }
...
async function seedDefaults() {
  ...
  if (userCount === 0) {
    ...
    await users().insertMany([
      { id: newId("u"), username: "admin", displayName: "Admin", role: "admin", passwordHash: adminPw.hash, passwordSalt: adminPw.salt, notify: DEFAULT_NOTIFY, createdAt: new Date().toISOString() },
      { id: newId("u"), username: "employee", displayName: "Employee", role: "employee", passwordHash: empPw.hash, passwordSalt: empPw.salt, notify: DEFAULT_NOTIFY, createdAt: new Date().toISOString() },
    ]);
  }
  ...
  if (!config) {
    await settingsCol().insertOne({
      ...
      // Sending account only - who receives what is each user's own notify prefs (see users collection).
      email: { enabled: false, smtpHost: "smtp.gmail.com", smtpPort: 587, smtpUser: "", smtpAppPassword: "" },
    });
  }
}
...
async function updateUserNotifyPrefs(id, notify) {
  await users().updateOne({ id }, { $set: { notify } });
...
async function getTasks() {
  const docs = await tasksCol().find({}).toArray();
  return docs.map(stripMongoId);
}

async function getTasksForUser(userId) {
  const docs = await tasksCol().find({ assignedTo: userId }).toArray();
  return docs.map(stripMongoId);
}

async function findTaskById(id) {
  const doc = await tasksCol().findOne({ id });
  return doc ? stripMongoId(doc) : null;
}
...
async function updateTask(id, fields) {
  await tasksCol().updateOne({ id }, { $set: fields });
...
async function getCallsForDay(userId, date) {
  const docs = await callsCol().find({ userId, date }).sort({ createdAt: 1 }).toArray();
  return docs.map(stripMongoId);
}

async function getCallsMonth(userId, year, month) {
  const prefix = `${year}-${String(month).padStart(2, "0")}`;
  const docs = await callsCol().find({ userId, date: { $regex: `^${escapeRegex(prefix)}` } }).toArray();
  return docs.map(stripMongoId);
}

async function findCallById(id) {
  const doc = await callsCol().findOne({ id });
  return doc ? stripMongoId(doc) : null;
}
...
module.exports = {
  connectDB, newId, DEFAULT_NOTIFY,
  getCheques, saveCheque, updateCheque, deleteCheque, replaceDeal,
  getUsers, findUserByUsername, findUserById, insertUser, updateUserPassword, updateUserNotifyPrefs, deleteUser, countAdmins,
  getConfig, saveConfig,
  wasReminderSent, markReminderSent,
  getTrackerEntry, upsertTrackerEntry, getTrackerMonth,
  getTasks, getTasksForUser, findTaskById, createTask, updateTask, deleteTask,
  getCallsForDay, getCallsMonth, findCallById, createCall, deleteCall,
};
